...
def data_import(request):  
    ...
    if request.method == 'POST':  
        ...
        cursor1.close() 
        sql1 = '' 
        return render_to_response('sjdr.html', {'errorrow':errorrow,'errortab':errortab,'username':username})         
...
left join ( select a.id,a.zhmc,a.rq,lead(a.rq,1,0) over(order by a.rq) last_rq,a.zhhj as zhhj,a.bdhzhhj from NJFX_SYL a) y on y.zhmc=x.zhmc and x.rq=y.last_rq where  x.zhmc like %s  and  x.rq>=%s  and x.rq<=%s order by x.rq desc''',[zhtemp,rq,rq2])
    ...
    return render_to_response('tghsj.html',{'sj':sj,'blx':blx[0][0],'rqlist':rqlist,'zhlist':zhlist,'kssj':rq,'jzsj':rq2,'rq':rq,'zh':zh,'username':username})
@login_required
def Tzqkfx(request):
    username = request.user.username
    cursorsj = connection.cursor()
